...
import com.lixiang.educationOnlie.qz.bean.{DwdQzPaperView, DwdQzPoint, DwdQzQuestion}
...
object EtlDataService {
   ...
   * @param ssc
   */
  def etlDataChapter(spark:SparkSession,ssc:SparkContext){
    import spark.implicits._
    val textRDD: RDD[String] = ssc.textFile("hdfs://cmaster.basichadoop.com:8020/user/atguigu/ods/QzChapter.log")
    textRDD.filter( line => {
  ...
  def etlQzChapterList(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
   ...
   * @param spark
   */
  def etlQzPoint(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
   ...
   * @param spark
   */
  def etlQzSiteCourse(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
   ...
   * @param spark
   */
  def etlQzWebsite(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
   ...
   * @param spark
   */
  def etlQzMajor(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
   ...
   * @param spark
   */
  def etlQzBusiness(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
  ...
  def etlQzQuestion(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
    ssc.textFile("/user/atguigu/ods/QzQuestion.log").filter(item => {
      ...
      partitions.map(item => {
        ...
        val score = BigDecimal.apply(jsonObject.getDoubleValue("score")).setScale(1, BigDecimal.RoundingMode.HALF_UP)//设置精度，并四舍五入（BigDecimal.RoundingMode.HALF_UP）
  ...
  def etlQzQuestionType(ssc: SparkContext, spark: SparkSession) = {
    import spark.implicits._
