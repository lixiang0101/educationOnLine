...
object UserPaperDetailDao {
  ...
  def getDwsQzCourse(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql("select " +
      "sitecourseid,siteid,courseid,sitecoursename,coursechapter,sequence as course_sequence," +
      "status as course_status,sitecourse_creator,sitecourse_createtime,helppaperstatus,servertype,boardid,showstatus,majorid," +
      s"coursename,isadvc,chapterlistid,pointlistid,courseeduid,edusubjectid,dn " +
      s"from dws.dws_qz_course where dt='$dt'")
  }
